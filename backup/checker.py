...
def scrape_jobs_detailed():
    ...
    try:
        with sync_playwright() as p:
            ...
            logger.info("Launching browser in visible (non-headless) mode for debugging...")
...
def add_recipient(email, notify_new=True, notify_reopened=True, notify_spotfreed=True, delay=0, phone_number=None, use_telegram=False, telegram_chat_id=None):
    """Add or update a recipient with Telegram support."""
...
def update_recipient(email, notify_new, notify_reopened, notify_spotfreed, delay, use_telegram=False, telegram_chat_id=None):
    """Update a recipient's preferences."""
    add_recipient(email, notify_new, notify_reopened, notify_spotfreed, delay, use_telegram=use_telegram, telegram_chat_id=telegram_chat_id)
...
def check_jobs():
    """Check for new jobs with improved state management."""
    lock_file = acquire_lock()
    if not lock_file:
        logger.warning("Skipping job check - another instance is running")
        return
    ...
    try:
        ...
        for key, info in new.items():
            ...
            # Check if we've already processed this job in a previous run
            ...
            if key not in old:
                ...
                logger.info(f"New job found: {key}")
                ...
                if prev.get('locked') and not info['locked']:
                    ...
                    logger.info(f"Job reopened: {key}")
                    state["processed_jobs"][job_hash] = run_id
                if (prev.get('filled') is not None and info['filled'] is not None
                    ...
                    logger.info(f"Spot freed in job: {key}")
                    state["processed_jobs"][job_hash] = run_id
